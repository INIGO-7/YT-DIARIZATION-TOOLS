...
import os
import sys
sys.path.append('/home/guayo/Desktop/programming/python/NLP/YT-WHISPER_SUMMARIZATION')
...
# Now download the audio from youtube
GetFromYoutube(video_url).get_audio_and_rename("input", ".wav")
...
split_wav_pydub('whisper-speaker-recognition/input_prep.wav', 600)  # Split by every 10 minutes

for transcription_name in os.listdir("whisper-speaker-recognition/audio_fragments"):
  if os.path.isfile(os.path.join("whisper-speaker-recognition/audio_fragments", transcription_name)):
    # Define chunk size and calculate the number of chunks
    FILE = {'uri': 'none', 'audio': transcription_name}
    dz = pipeline(FILE)
